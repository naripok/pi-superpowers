...
Brainstorming reads the living spec
    → Planning writes a delta spec
    → Implementation follows the plan
    → Finishing syncs the delta into the living spec
```

The living spec is the starting point for every new feature — it tells you what exists before you design what comes next.
...
1. **Read living specs** — check `docs/specs/` for relevant domain specs. They describe current behavior. If none exist, note that one will be created later.
...
6. **Write design doc** → `docs/design/YYYY-MM-DD-<topic>-design.md`
7. **Spec self-review** — placeholders, consistency, scope, ambiguity
8. **User reviews written spec**
9. **Transition** → invoke `writing-plans`

**Hard gate:** No code, no implementation skills, no scaffolding until the design is approved.

### 2. Writing Plans (`writing-plans/SKILL.md`)

Convert the approved design into an exhaustive implementation plan and a delta spec.

1. **Read the design doc**
2. **Read relevant living specs** from `docs/specs/` (current behavior for affected domains)
3. **File structure** — map which files are created/modified and what each is responsible for
4. **Write tasks** — bite-sized (2–5 min), TDD steps (write test → verify fail → implement → verify pass → commit)
5. **Write delta spec** → `docs/plans/YYYY-MM-DD-<feature>-delta.md`
...
6. **Self-review** — 5 checks:
   ...
   - Delta coverage (delta spec → tasks — every ADDED/MODIFIED requirement has a test)
   - Reverse coverage (tasks → delta + design — no scope creep)
   - Placeholder scan
   - Type consistency
...
2. Dispatch **spec compliance reviewer** → checks against:
   - **Delta spec** (primary) — behavioral contract
   - **Design doc** (context) — intent and rationale
...
│   └── 2026-05-06-add-webpush-design.md
└── plans/
    ├── 2026-05-06-add-webpush.md          # Implementation plan
    └── 2026-05-06-add-webpush-delta.md    # Delta spec (behavioral contract)
```

| Path                                   | What                                      | Lifespan                              |
| -------------------------------------- | ----------------------------------------- | ------------------------------------- |
| `docs/specs/<domain>.md`               | How the system _currently_ behaves        | Persistent, updated by sync           |
| `docs/design/<date>-<topic>-design.md` | What we're planning to build              | One-off per feature                   |
...
| Case                                   | Handling                                                                                                          |
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| **Cold start** (no living specs)       | Brainstorming notes "no specs found." First delta spec is all ADDED. Sync creates the living spec from scratch.   |
| **Non-behavioral changes**             | Delta declares "No Behavioral Changes." Sync skips living spec updates. Normal SDD flow still applies.            |
| **Multi-domain features**              | Delta has multiple `## Domain: <name>` sections. Sync iterates over each, updating the corresponding living spec. |
